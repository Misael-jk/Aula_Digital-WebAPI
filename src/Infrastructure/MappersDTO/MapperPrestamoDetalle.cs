    ...
    public IEnumerable<PrestamosDetalleDTO> GetByIdDTO(int idPrestamo, int? idCarrito)
    {
        ...
        var sql = @"SELECT
                    ...
                    CASE WHEN EXISTS (
                        ...
                        WHERE dd.idElemento = e.idElemento
                          AND d.idPrestamo = @idPrestamo
                    ) THEN 1 ELSE 0 END AS HasValue,

                    (
                        SELECT dd.observaciones
                        FROM DevolucionDetalle dd
                        INNER JOIN Devoluciones d ON d.idDevolucion = dd.idDevolucion
                        WHERE dd.idElemento = e.idElemento
                          AND d.idPrestamo = @idPrestamo
        ...
        return Conexion.Query<Elemento, TipoElemento, Notebooks, Carritos, int, string, PrestamosDetalleDTO>(
            sql,
            (elemento, tipo, notebook, carrito, hasValue, observaciones) =>
                new PrestamosDetalleDTO
                {
                    ...
                    Devuelto = (hasValue == 1),
                    ObservacionDevolucion = observaciones
                },
            parameters,
            splitOn: "ElementoTipo,Equipo,EquipoCarrito,HasValue,Observaciones"
        ).ToList();
    }
